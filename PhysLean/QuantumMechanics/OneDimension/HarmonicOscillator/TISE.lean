...
lemma deriv_deriv_eigenfunction_succ (n : ℕ) (x : ℝ) :
    ...
    Complex.ofReal (1/√(2 ^ (n + 1) * (n + 1) !)) *
      ...
      (physHermite (n + 1) (√(Q.m * Q.ω / Q.ℏ) * x))) * Q.eigenfunction 0 x) := by
...
lemma deriv_deriv_eigenfunction_one (x : ℝ) :
  ...
  congr 2
  simp [physHermite_one, Polynomial.aeval]
...
lemma deriv_deriv_eigenfunction_succ_succ (n : ℕ) (x : ℝ) :
  ...
  trans Complex.ofReal (1/Real.sqrt (2 ^ (n + 1 + 1) * (n + 1 + 1) !)) *
        (((- Q.m * Q.ω / Q.ℏ) * (2 * (n + 2)
        + (1 + (-(Q.m * Q.ω) / Q.ℏ) * x ^ 2)) *
        (physHermite (n + 2) (√(Q.m * Q.ω / Q.ℏ) * x))) * Q.eigenfunction 0 x)
  · rw [deriv_deriv_eigenfunction_succ]
    congr 2
    trans (√(Q.m * Q.ω / Q.ℏ) * 2 * (n + 1 + 1) * (√(Q.m * Q.ω / Q.ℏ) *
      2 * (n + 1) * (physHermite n (√(Q.m * Q.ω / Q.ℏ) * x))) +
      (-(Q.m * Q.ω) / Q.ℏ) * √(Q.m * Q.ω / Q.ℏ) * (4 * (n + 1 + 1) * x) *
      (physHermite (n + 1) (√(Q.m * Q.ω / Q.ℏ) * x)) +
      (-(Q.m * Q.ω) / Q.ℏ) * (1 + (-(Q.m * Q.ω) / Q.ℏ) * x ^ 2) *
      (physHermite (n + 2) (√(Q.m * Q.ω / Q.ℏ) * x)))
    · rw [deriv_physHermite_succ]
      simp
    ...
    trans (- Q.m * Q.ω / Q.ℏ) * (2 * (n + 1 + 1) *
          (2 * (√(Q.m * Q.ω / Q.ℏ) * x) * (physHermite (n + 1) (√(Q.m * Q.ω / Q.ℏ) * x)) -
          2 * (n + 1) * (physHermite n (√(Q.m * Q.ω / Q.ℏ) * x)))
          + (1 + (-(Q.m * Q.ω) / Q.ℏ) * x ^ 2) * (physHermite (n + 2) (√(Q.m * Q.ω / Q.ℏ) * x)))
    · ring
    trans (- Q.m * Q.ω / Q.ℏ) * (2 * (n + 1 + 1) * (physHermite (n + 2) (√(Q.m * Q.ω / Q.ℏ) * x))
          + (1 + (-(Q.m * Q.ω) / Q.ℏ) * x ^ 2) * (physHermite (n + 2) (√(Q.m * Q.ω / Q.ℏ) * x)))
    · congr
      conv_rhs =>
        rw [physHermite_succ]
      simp only [nsmul_eq_mul, cast_ofNat, derivative_physHermite, add_tsub_cancel_right, cast_add,
        ...
        Complex.ofReal_one]
      simp only [Polynomial.aeval, Polynomial.eval₂AlgHom'_apply, Polynomial.eval₂_ofNat,
        Complex.ofReal_ofNat, sub_right_inj]
      ring
    ring
